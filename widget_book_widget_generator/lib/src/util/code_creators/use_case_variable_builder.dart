...
class UseCaseVariableBuilder {
  ...
  static Reference _buildKnob({required WidgetParameter parameter, required WidgetField field, String? path}) {
    ...
    final knob = switch (parameter.type.type) {
      ...
      DataTypeEnum.color => "context.knobs.color${isNullable ? 'OrNull' : ''}(label: '$knobName', initialValue: DateTime.now() )",
      DataTypeEnum.date => "context.knobs.dateTime${isNullable ? 'OrNull' : ''}(label: '$knobName', initialValue:${defaultValue}, start: DateTime(1950, 1,1, 0, 0, 0), end: DateTime(2050, 1, 1, 0, 0, 0))",
